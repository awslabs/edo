...
async fn discover_auth(registry: &RegistryUri) -> crate::Result<Option<Token>> {
    ...
    if registry.base().starts_with("public.ecr.aws") {
        ...
        Ok(ecr_response
            .authorization_data()
            .and_then(|x| x.authorization_token.clone().map(Token::Bearer)))
