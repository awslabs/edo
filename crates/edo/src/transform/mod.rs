...
pub trait Transform {
    ...
    ///
    /// Implementations that don't need short-circuiting should just return
    /// `Ok(true)` so plugin transforms keep their existing behaviour.
    /// Builtins that only fetch sources in `prepare` override this to
    /// short-circuit when every input source reports cached.
