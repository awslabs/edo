...
/// Validate a staging `path_hint` and return it as a relative path safe to
/// join beneath a destination root.
///
/// A hint is untrusted input — it is deserialized from an artifact's config
/// (see [`crate::storage::Config::path_hints`]) which is produced by
/// arbitrary sources and transforms. Naively joining it onto a destination
/// path with [`Path::join`] would let an absolute hint replace the base
/// and `..` components escape it. Both would allow a staged artifact to
/// write outside of `options.path()`, clobbering unrelated files.
///
/// This helper rejects any [`Component::RootDir`], [`Component::Prefix`],
/// or [`Component::ParentDir`] and strips [`Component::CurDir`]. It
/// mirrors the existing `sanitize_zip_path` policy used when extracting
/// archives (`crates/core/src/environment/mod.rs`) so hint-driven writes
/// obey the same containment rule as zip entries.
///
/// A hint of `"."` (or one that reduces to empty after stripping curdir
/// components) is the canonical way to say "stage at the artifact root
/// with no subdirectory" and is returned as an empty [`PathBuf`]. Joining
/// an empty path with [`Path::join`] leaves the base unchanged, which is
/// exactly the desired behaviour.
pub fn sanitize_path_hint(hint: &Path) -> Option<PathBuf> {
...
impl Environment {
    ...
    pub async fn stage(&self, ctx: &Handle, options: ArtifactStageOptions) -> EnvResult<()> {
        ...
        for layer in artifact.layers() {
            ...
            // A hint is untrusted input drawn from the artifact config;
            // reject anything that would escape `options.path()` before
            // handing it to the environment. See `sanitize_path_hint`.
            let safe_hint = if !options.ignore_artifact_path()
                && let Some(hint) = hint.as_ref()
            {
                match sanitize_path_hint(hint) {
                    Some(safe) => Some(safe),
                    None => {
                        return error::UnsafePathHintSnafu { hint: hint.clone() }.fail();
                    }
                }
            } else {
                None
            ...
            if layer.media_type().is_archive() && options.extract() {
                let path = if let Some(safe) = safe_hint {
                    let resolved = resolved_stage_subpath(&safe, layer.media_type());
                ...
                if !options.ignore_artifact_path() {
                    let filename = safe_hint.unwrap_or(PathBuf::from(layer.digest().digest()));
...
mod tests {
    ...
    fn sanitize_accepts_plain_relative_path() {
        assert_eq!(
            sanitize_path_hint(Path::new("vendor/foo.tar.gz")),
            Some(PathBuf::from("vendor/foo.tar.gz"))
        );
    }

    #[test]
    fn sanitize_strips_leading_curdir() {
        assert_eq!(
            sanitize_path_hint(Path::new("./vendor/foo")),
    ...
    fn sanitize_rejects_absolute_path() {
        assert!(sanitize_path_hint(Path::new("/etc/passwd")).is_none());
    }

    #[test]
    fn sanitize_rejects_parent_dir() {
        assert!(sanitize_path_hint(Path::new("../escape")).is_none());
    }

    #[test]
    fn sanitize_rejects_embedded_parent_dir() {
        // `foo/../../bar` normalises to `../bar` — reject.
        assert!(sanitize_path_hint(Path::new("foo/../../bar")).is_none());
    }

    #[test]
    fn sanitize_treats_dot_as_root_hint() {
        // `"."` is the canonical "stage at artifact root" hint and must
        // be accepted; it collapses to an empty path that joins as a no-op.
        assert_eq!(sanitize_path_hint(Path::new(".")), Some(PathBuf::new()));
        assert_eq!(sanitize_path_hint(Path::new("")), Some(PathBuf::new()));
    }
}
