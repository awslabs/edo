...
use edo::environment::sanitize_path_hint;
        ...
        for layer in artifact.layers() {
            ...
            match media_type {
                ...
                MediaType::File(compression) => {
                    // Sanitize any recorded hint before joining onto the
                    // output root — it comes from artifact config which is
                    // untrusted and could otherwise be absolute or contain
                    // `..` components and escape `self.output`.
                    let filename = match artifact.config().path_hint_for(layer.digest()) {
                        Some(hint) => sanitize_path_hint(hint)
                            .ok_or_else(|| error::Error::UnsafePathHint { hint: hint.clone() })?,
                        None => PathBuf::from(layer.digest().digest()),
                    };
